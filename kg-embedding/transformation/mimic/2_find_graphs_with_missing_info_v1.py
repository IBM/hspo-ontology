...
def find_graphs_with_missing_info(input_path):
    unique_t = [["patient", "hasDemographics", "demographics"], 
                ["patient", "hasDisease", "diseases"], 
                ["patient", "hasIntervention", "interventions"], 
                ["interventions", "hasIntervention", "procedure_provisioning_ICD9"], 
                ["interventions", "hasIntervention", "medication_provisioning"], 
    ...
    g0 = find_json_files(input_path + 'undirected/0/v1/')
    for p in g0:
        flag = 0
        g = read_json(p)
        uniq_tmp = []
    ...
    g1 = find_json_files(input_path + 'undirected/1/v1/')
    for p in g1:
        flag = 0
        g = read_json(p)
        uniq_tmp = []
...
if __name__ == '__main__':
    ...
    parser.add_argument("--input_path", default='data/triplet_format_graphs/', type=str, required=False,
                        help = "The input path with the graphs (version 1 undirected).")

    args = parser.parse_args()

    t_dict_0, t_dict_1, black_list_graphs_0, black_list_graphs_1 = find_graphs_with_missing_info(args.input_path)
    save_json(t_dict_0, 'missing_info_dict_graph_0_v1_undirected.json')
    save_json(t_dict_1, 'missing_info_dict_graph_1_v1_undirected.json')
    save_json(black_list_graphs_0, 'missing_info_bl_graph_0_v1_undirected.json')
    save_json(black_list_graphs_1, 'missing_info_bl_graph_1_v1_undirected.json')